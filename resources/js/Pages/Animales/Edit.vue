<template>
    <Head title="Editar animales" />

    <AuthenticatedLayout>
        <div class="container bg-white mx-auto">
        <div class="flex justify-between items-center bg-yellow-950 p-4">
            <h1 class="text-2xl font-normal text-white">Editar animales</h1>
        </div>
        <div class="overflow-x-auto bg-white py-5 px-6">
            ...
            <div class="mb-4 flex justify-between items-center">
                ...
                <div v-if="form.recentlySuccessful"  class="w-1/3 flex justify-center bg-green-200 px-4 py-2 rounded">
                    ...
                    <span class="text-green-800">Animal Actualizado</span>
                </div>
                </Transition>
...
const page = usePage();
const animal = ref(page.props.animal);
const props = defineProps({
  animal: Object,
  razas: Array,
});
const form = useForm({
            arete: animal.value.arete,
            ingreso: animal.value.ingreso,
            nombre_animal: animal.value.nombre_animal,
            fecha: animal.value.fecha,
            sexo: animal.value.sexo,
            clasificacion: animal.value.clasificacion,
            precio_compra: animal.value.precio_compra,
            estado: animal.value.estado,
            raza_id: animal.value.raza_id,
        });
const submit = () =>{
    form.post(route('animales.update', animal.value), {
        onSuccess: (e) => {
