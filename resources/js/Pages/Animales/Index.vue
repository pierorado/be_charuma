<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="flex justify-between items-center bg-yellow-950 p-4">
            <h1 class="text-2xl font-normal text-white">Animales</h1>
            <Link :href="route('animales.create')" class="bg-orange-600 text-white px-8 py-2 rounded">Agregar</Link>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 border border-gray-200">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center font-medium border-r border-gray-200">Sexo</th>
                        <th class="px-6 py-3 text-center font-medium border-r border-gray-200">Raza</th>
                        <th class="px-6 py-3 text-center font-medium border-r border-gray-200">Clasificación</th>
                        <th class="px-6 py-3 text-center font-medium border-r border-gray-200">Acciones</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="animal in animales.data" :key="animal.id_animal">
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ animal.arete }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ animal.nombre_animal }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ animal.sexo }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ animal.raza.nombre_raza }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ animal.clasificacion}}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">

                            <Link :href="route('animales.show', animal.id_animal)" class="bg-green-500 text-white px-2 py-1 rounded mr-2 items-center inline-flex">
                            ...
                            <Link :href="route('animales.edit', animal.id_animal)" class="bg-blue-500 text-white px-2 py-1 rounded mr-2 items-center inline-flex">
                                <svg width="14" height="12" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <Link @success="onDeleteSuccess" :href="route('animales.destroy', animal.id_animal)" method="delete" as="button" class="bg-red-500 text-white px-2 py-1 rounded">
                                <svg width="14" height="12" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const page = usePage();
const animales = ref(page.props.animales);

const onDeleteSuccess = (e) => {
    animales.value = e.props.animales;
}
</script>
