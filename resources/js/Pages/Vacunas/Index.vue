<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="flex justify-between items-center bg-yellow-950 p-4">
            <h1 class="text-2xl font-normal text-white">Vacunas</h1>
            <Link :href="route('vacunas.create')" class="bg-orange-600 text-white px-8 py-2 rounded">Agregar</Link>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 border border-gray-200">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center font-medium border-r border-gray-200">Vacuna</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="vacuna in vacunas.data" :key="vacuna.id_vacuna ">
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ vacuna.animal.arete }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ vacuna.animal.nombre_animal }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ vacuna.nombre_vacuna }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ formatDate(vacuna.Fecha_administrada) }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ formatDate(vacuna.Fecha_programada) }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">

                            <Link :href="route('vacunas.show', vacuna.id_vacuna)" class="bg-green-500 text-white px-2 py-1 rounded mr-2 items-center inline-flex">
                            ...
                            <Link :href="route('vacunas.edit', vacuna.id_vacuna)" class="bg-blue-500 text-white px-2 py-1 rounded mr-2 items-center inline-flex">
                                <svg width="14" height="12" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <Link @success="onDeleteSuccess" :href="route('vacunas.destroy', vacuna.id_vacuna)" method="delete" as="button" class="bg-red-500 text-white px-2 py-1 rounded">
                                <svg width="14" height="12" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="flex flex-col items-center bg-white px-5 py-5 xs:flex-row xs:justify-between">
                <pagination :links="vacunas.links" />
        </div>
    </div>
    </AuthenticatedLayout>
</template>
...
const page = usePage();
const vacunas = ref(page.props.vacunas);

const onDeleteSuccess = (e) => {
    vacunas.value = e.props.vacunas;
}
