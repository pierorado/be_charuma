<template>
    <Head title="Añadir Gestante" />

    <AuthenticatedLayout>
        <div class="container bg-white mx-auto">
        <div class="flex justify-between items-center bg-yellow-950 p-4">
            <h1 class="text-2xl font-normal text-white">Añadir Gestante</h1>
        </div>
...
const submit = () =>{
    form.post(route('gestantes.store'),{
        onSuccess: (e) => {
...
watch(() => form.Fecha_prenez, (newFechaPrenez) => {
  if (newFechaPrenez) {
    ...
    fechaPrenez.setMonth(fechaPrenez.getMonth() + 9);
