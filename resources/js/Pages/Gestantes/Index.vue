<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="flex justify-between items-center bg-yellow-950 p-4">
            <h1 class="text-2xl font-normal text-white">Gestantes</h1>
            <Link :href="route('gestantes.create')" class="bg-orange-600 text-white px-8 py-2 rounded">Agregar</Link>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 border border-gray-200">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center font-medium border-r border-gray-200">Nombre del animal</th>
                        <th class="px-6 py-3 text-center font-medium  border-r border-gray-200">Fecha de preñez</th>
                        <th class="px-6 py-3 text-center font-medium border-r border-gray-200">Fecha de estimada</th>
                        <th class="px-6 py-3 text-center font-medium border-r border-gray-200">Fecha de parto</th>
                        <th class="px-6 py-3 text-center font-medium border-r border-gray-200">Acciones</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="gestante in gestantes.data" :key="gestante.id_gestacion">
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ gestante.animal.arete }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ gestante.animal.nombre_animal }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ formatDate(gestante.Fecha_prenez) }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ formatDate(gestante.Fecha_estimada) }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">{{ formatDate(gestante.Fecha_parto) }}</td>
                        <td class="px-6 py-2 text-center border-r border-gray-200">

                            <Link :href="route('gestantes.show', gestante.id_gestacion)" class="bg-green-500 text-white px-2 py-1 rounded mr-2 items-center inline-flex">
                            ...
                            <Link :href="route('gestantes.edit', gestante.id_gestacion)" class="bg-blue-500 text-white px-2 py-1 rounded mr-2 items-center inline-flex">
                                <svg width="14" height="12" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <Link @success="onDeleteSuccess" :href="route('gestantes.destroy', gestante.id_gestacion)" method="delete" as="button" class="bg-red-500 text-white px-2 py-1 rounded">
                                <svg width="14" height="12" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="flex flex-col items-center bg-white px-5 py-5 xs:flex-row xs:justify-between">
                <pagination :links="gestantes.links" />
        </div>
    </div>
    </AuthenticatedLayout>
</template>
...
const page = usePage();
const gestantes = ref(page.props.gestantes);

const onDeleteSuccess = (e) => {
    gestantes.value = e.props.gestantes;
}
